/**
 * Three.jsとcannon-esを用いた球体宇宙要塞の爆発シーン。
 * Spaceキーで爆発を開始し、Rキーで初期状態へ戻す。
 */
...
// 型定義
type FortressState =
    "idle" | "charging" | "charged" | "flash" | "explosion" | "aftermath";

type BasicSphereMesh = THREE.Mesh<
...
const easeOut = (progress: number, power = 3): number =>
    1 - Math.pow(1 - progress, power);
...
class ThreeJSContainer {
    ...
    // 火花・破片・爆煙
    // ------------------------------------------------------------------
    private sparkTrails!: THREE.LineSegments<
    ...
    private createFortress = (): void => {
        ...
        const body = new THREE.Mesh(
            new THREE.SphereGeometry(CONFIG.fortressRadius, 64, 64),
            this.bodyMaterial,
        );
    ...
    // 火球
    // ------------------------------------------------------------------
    // 正二十面体の頂点を波形で変形し、不規則な爆炎の輪郭を作る。
    private createIrregularFireballGeometry = (
        roughness: number,
        frequency: number,
        seed: number,
    ): THREE.BufferGeometry => {
        ...
        for (let i = 0; i < position.count; i++) {
            ...
            const waveB = Math.sin(
                (vertex.x + vertex.y + vertex.z) * frequency * 1.9 + seed * 2.3,
            );
    ...
    private createExplosionEffect = (): void => {
        this.fireballs = [
            ...
            this.createFireballLayer(
                this.createIrregularFireballGeometry(0.10, 6.5, 2.4),
                0xffde75,
                19,
                2.55,
                0.46,
    ...
    private updateExplosion = (deltaTime: number): void => {
        ...
        for (const layer of this.fireballs) {
            ...
            const localProgress = clamp01(
                (progress - layer.delay) / (1 - layer.delay),
            );
            ...
            const pulse =
                1 +
                Math.sin(this.explosionElapsed * layer.pulseSpeed + layer.pulseOffset) *
                layer.pulseAmount;

            const scale = (0.08 + growth * layer.maxScale) * pulse;

            layer.mesh.scale.set(
                scale * layer.stretch.x,
                scale * layer.stretch.y,
                scale * layer.stretch.z,
            );
    ...
    private startShockwave = (): void => {
        ...
        this.shockwaveGroup.quaternion.copy(
            this.camera.quaternion
        );

        this.shockwaveGroup.rotateZ(
        ...
        for (const layer of this.shockwaves) {
            ...
            layer.mesh.scale.set(
                0.05 * layer.stretch.x,
                0.05 * layer.stretch.y,
                0.05
            );
        }
    };
    ...
    private updateShockwave = (deltaTime: number): void => {
        ...
        for (const layer of this.shockwaves) {
            ...
            const fadeOut =
                1 - THREE.MathUtils.smoothstep(localProgress, layer.fadeStart, 1);
    ...
    // 火花
    // ------------------------------------------------------------------
    private getSparkPositionAttribute = (): THREE.BufferAttribute =>
        this.sparkTrails.geometry.getAttribute("position") as THREE.BufferAttribute;

    private getSparkPositions = (): Float32Array =>
        this.getSparkPositionAttribute().array as Float32Array;
    ...
    private createSparkEffect = (): void => {
        ...
        for (let i = 0; i < CONFIG.sparkCount; i++) {
            ...
            const headColor = new THREE.Color(
                colorValue < 0.2 ? 0xffffff : colorValue < 0.65 ? 0xffdb55 : 0xff6a18,
            );
    ...
    private startSparkEffect = (): void => {
        ...
        for (let i = 0; i < CONFIG.sparkCount; i++) {
            ...
            const startPosition = direction
                .clone()
                .multiplyScalar(THREE.MathUtils.randFloat(0, 0.3));
    ...
    private updateSparkEffect = (deltaTime: number): void => {
        ...
        this.sparkTrails.material.opacity =
            1 - THREE.MathUtils.smoothstep(progress, 0.15, 1);
    ...
    // cannon-esの物理BodyをThree.jsのMeshへ同期する。
    ...
    private startDebrisEffect = (): void => {
        ...
        for (const item of this.debrisItems) {
            ...
            body.position.set(
                direction.x * startRadius,
                direction.y * startRadius,
                direction.z * startRadius,
            );
            body.velocity.set(0, 0, 0);
            body.angularVelocity.set(0, 0, 0);
            body.force.set(0, 0, 0);
            ...
            body.applyImpulse(
                new CANNON.Vec3(
                    direction.x * speed * body.mass,
                    direction.y * speed * body.mass,
                    direction.z * speed * body.mass,
                ),
                new CANNON.Vec3(
    ...
    private updateDebrisEffect = (deltaTime: number): void => {
        ...
        this.debrisMaterial.opacity =
            1 - THREE.MathUtils.smoothstep(progress, 0.58, 1);
    ...
    private updateSmokeEffect = (deltaTime: number): void => {
        ...
        for (let i = 0; i < this.smokeParticles.length; i++) {
            ...
            particle.sprite.position.x +=
                Math.sin(localTime * 2.1 + i) * 0.015 * deltaTime;
            particle.sprite.position.y +=
                Math.cos(localTime * 1.7 + i) * 0.015 * deltaTime;
            particle.sprite.material.rotation += particle.rotationSpeed * deltaTime;

            const growth = easeOut(progress);
            const scale =
                THREE.MathUtils.lerp(particle.startScale, particle.endScale, growth) *
                (1 + Math.sin(localTime * 3 + i) * 0.06);
            ...
            particle.sprite.material.opacity =
                particle.baseOpacity * fadeIn * fadeOut;

            if (progress < 0.3) {
                particle.sprite.material.color
                    .copy(COLORS.smokeHot)
                    .lerp(COLORS.smokeMiddle, progress / 0.3);
            } else {
                particle.sprite.material.color
                    .copy(COLORS.smokeMiddle)
                    .lerp(COLORS.smokeCold, (progress - 0.3) / 0.7);
            }
        }
    ...
    private resetFortress = (): void => {
        ...
        this.debrisItems.forEach((item, index) =>
            this.resetDebrisBody(item, this.getDebrisResetX(index)),
        );
    ...
    private startAnimation = (): void => {
        const animate = (): void => {
            ...
            if (this.fortressGroup.visible) {
                this.fortressGroup.rotation.y +=
                    deltaTime * CONFIG.fortressRotationSpeed;
            }
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    guide.textContent = "Space：エネルギー充填　R：リセット";
